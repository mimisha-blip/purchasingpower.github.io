...
test('uses user-friendly summary language without scam wording', () => {
  const advice = buildTravelPriceAdvice({
    item: 'lunch',
    destinationCity: 'New York',
    destinationCountry: 'USA',
...
test('uses product context for iPhone instead of food context', () => {
  const input = buildAdvisorInputFromCountries({
    item: 'iPhone',
    destinationCountry: {
      country_name: 'United States',
...
test('returns summary-focused advice without step-by-step output', () => {
  const advice = buildTravelPriceAdvice({
    ...
    homeCurrency: 'INR',
    exchangeRate: 83,
    affordabilityScore: 19980,
    destinationTypicalRange: { min: 799, max: 1199 },
    homeTypicalRange: { min: 70000, max: 160000 }
  });
